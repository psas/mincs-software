...
class ControlPanel(tk.Tk):
    def __init__(self, labjack_handler, pin_config, initial_pin_values, loading_window):
    ...
    def create_test_tab(self):
        ...
        firing_sequence_input = self.create_input_box(test_tab, self.firing_sequence_var, "Firing Sequence Duration (sec):", "firing_sequence_time")

        # Ignition Sequence duration input box
        self.ignition_sequence_var = tk.StringVar(value="")
        ignition_sequence_input = self.create_input_box(test_tab, self.ignition_sequence_var, "Ignition Sequence Duration (sec):", "ignition_sequence_time")

        # Purge Sequence duration input box
        self.purge_var = tk.StringVar(value="")
        purge_sequence_input = self.create_input_box(test_tab, self.purge_var, "Purge Sequence Duration (sec):", "purge_sequence_time")

        # Shutdown Sequence duration input box
        self.shutdown_sequence_var = tk.StringVar(value="")
        shutdown_sequence_input = self.create_input_box(test_tab, self.shutdown_sequence_var, "Shutdown Sequence Duration (sec):", "shutdown_sequence_time")
    ...
    def validate_float_input(self, input_str):
        try:
            ...
            messagebox.showerror("Invalid Input", "PLEASE INPUT POSITIVE NUMBER ONLY")
            return False

    def create_input_box(self, parent, variable, label_text, attribute_name):
        frame = ttk.Frame(parent)
        frame.pack(side=tk.TOP, pady=(10, 0), padx=10, fill=tk.X)

        label = ttk.Label(frame, text=label_text, font=("TkDefaultFont", 8, "bold"))
        label.pack(side=tk.LEFT)

        input_field = ttk.Entry(frame, textvariable=variable)
        input_field.pack(side=tk.RIGHT)
        input_field.bind("<Return>", lambda event: (
            setattr(self, attribute_name, float(event.widget.get())),
            self.update_test_status_bar(label_text.upper() + " UPDATED"),
            self.update_button_states()
        ) if self.validate_float_input(event.widget.get()) else None)

        return input_field

    def check_inputs(self):
        try:
            # If any of these variables is an empty string or cannot be converted to an int,
            # int() will raise a ValueError and the function will return False.
            int(self.firing_sequence_var.get())
            int(self.ignition_sequence_var.get())
            int(self.purge_var.get())
            int(self.shutdown_sequence_var.get())
        except ValueError:
            return False
        return True
    ...
    def reset_test_inputs(self):
        ...
        self.purge_var.set("")
        self.shutdown_sequence_var.set("")
    ...
    def purge_thread(self):
        print("Purge Thread")
        self.message_queue.put(("ENGINE PURGE...", "black", ("Helvetica", 12, "bold")))
        ...
        start_time = time.time()
        while time.time() - start_time < self.purge_sequence_time:
            # Write the pins again to make sure their state doesn't change
            ljm.eWriteName(self.labjack_handler.handle, self.pin_config["ACTUATORS"]["LOX Valve"], 1)
            ljm.eWriteName(self.labjack_handler.handle, self.pin_config["ACTUATORS"]["N2 IPA Valve"], 1)
            ljm.eWriteName(self.labjack_handler.handle, self.pin_config["ACTUATORS"]["N2 LOX Valve"], 1)
    ...
    def shutdown_thread(self):
        ...
        start_time = time.time()
        while time.time() - start_time < self.shutdown_sequence_time:
            # Write the pins again to make sure their state doesn't change
            ljm.eWriteName(self.labjack_handler.handle, self.pin_config["ACTUATORS"]["LOX Valve"], 1)
            ljm.eWriteName(self.labjack_handler.handle, self.pin_config["ACTUATORS"]["N2 IPA Valve"], 1)
            ljm.eWriteName(self.labjack_handler.handle, self.pin_config["ACTUATORS"]["N2 LOX Valve"], 1)
...
if __name__ == "__main__":
    ...
    control_panel = ControlPanel(labjack_handler, pin_config)
