...
class ControlPanel(tk.Tk):
    def __init__(self, arduino_handler, labjack_handler, pin_config, initial_pin_values, loading_window=None):
        ...
        # self.arduino_handler = ArduinoHandler(self)
    ...
    def update_realtime_data_tab(self):
        for pin in range(0, 5):
            ...
            if pin in [0, 1, 2, 3]:
                ...
                temperature = (ain_value * 3750 / 65536 - 500) / 10
        ...
        self.realtime_values["Load1"].config(text=f"{load1:.2f} lbs")

        load2 = ((self.labjack_handler.read_analog_input(8)) / (0.02 * self.labjack_handler.read_analog_input(11))-24)
        self.realtime_values["Load2"].config(text=f"{load2:.2f} lbs")
    ...
    def update_indicator(self, sensor, status):
        if sensor in self.pin_config["ACTUATORS"]:
            if sensor in ["LOX Valve", "N2 IPA Valve", "N2 LOX Valve", "IPA Valve", "Ignition"]:
                status_text = "ON" if status == 0 else "OFF"
    ...
    def log_data(self, filename):
        while self.data_logging:
            ...
            # Get the current timestamp and add a new row to the CSV file
            current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            with open(filename, 'a', newline='') as csvfile:
                csv_writer = csv.writer(csvfile)
                csv_writer.writerow([current_time, load1, load2, *pressures, temperature])

            # Wait for one second before logging the next set of data
            time.sleep(1)
